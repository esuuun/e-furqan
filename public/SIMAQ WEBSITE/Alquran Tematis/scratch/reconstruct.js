...
// The exact target line where the corruption starts:
// line 5289 ends with: onclick="lookupVerseInSearch(${v.surah_num}, ${v.ayat_num}, event)" title="Lihat di Pencari Surat & Ayat">
// line 5290 is: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke= ...
// And the corruption ends right before:
// function switchMainMode(mode) {

const corruptStart = `                                            <button class="tts-button" style="color: #38bdf8; border-color: #38bdf8; background-color: rgba(56, 189, 248, 0.1);" onclick="lookupVerseInSearch(\${v.surah_num}, \${v.ayat_num}, event)" title="Lihat di Pencari Surat & Ayat">
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke=`;

const corruptEndMarker = `        function switchMainMode(mode) {`;

const startIndex = html.indexOf(corruptStart);
if (startIndex === -1) {
    console.error("corruptStart not found!");
    process.exit(1);
}

// Find the switchMainMode right after corruptStart
const endIndex = html.indexOf(corruptEndMarker, startIndex);
if (endIndex === -1) {
    console.error("corruptEndMarker not found!");
    process.exit(1);
}

console.log('Corrupt block found from character', startIndex, 'to', endIndex);
console.log('Replacing...');

const cleanBlock = `                                            <button class="tts-button" style="color: #38bdf8; border-color: #38bdf8; background-color: rgba(56, 189, 248, 0.1);" onclick="lookupVerseInSearch(\${v.surah_num}, \${v.ayat_num}, event)" title="Lihat di Pencari Surat & Ayat">
...
const newHtml = html.substring(0, startIndex) + cleanBlock + html.substring(endIndex);

// Also check switchMainMode to make sure translatePageContent() is called at the end
const oldSwitchMode = `            } else if (mode === 'keyword') {
...
const newSwitchMode = `            } else if (mode === 'keyword') {
                if (tabKeyword) tabKeyword.classList.add('active');
                if (keywordNav) keywordNav.style.display = 'block';
                if (keywordView) keywordView.style.display = 'block';
                const kwInput = document.getElementById('keyword-search-input');
            ...
            // Immediately update translations for the newly activated mode
            translatePageContent();
        }`;

let finalHtml = newHtml;
if (finalHtml.includes(oldSwitchMode)) {
    finalHtml = finalHtml.replace(oldSwitchMode, newSwitchMode);
    console.log('Updated switchMainMode to call translatePageContent()');
}

// Check initData to make sure translatePageContent() is called before checkAutoNav()
const oldInit = `            populateSelect(elTema, naturalSort(Object.keys(data)), "Pilih Tema");
            translateDropdowns();
            updateTranslatorInfo();
            checkAutoNav();
            translatePageContent();`;

const newInit = `            populateSelect(elTema, naturalSort(Object.keys(data)), "Pilih Tema");
            translateDropdowns();
            updateTranslatorInfo();
            translatePageContent();
            checkAutoNav();`;

if (finalHtml.includes(oldInit)) {
    finalHtml = finalHtml.replace(oldInit, newInit);
    console.log('Updated initData order');
}

// Test JavaScript syntax in finalHtml
const scriptMatches = [...finalHtml.matchAll(/<script[\s\S]*?>([\s\S]*?)<\/script>/gi)];
console.log('Found', scriptMatches.length, 'scripts. Testing syntax...');
...
if (!hasError) {
    ...
    console.log('SUCCESS: index.html updated and verified with 0 syntax errors!');
} else {
    console.error('ABORTING: Not saving index.html due to syntax errors.');
    process.exit(1);
}

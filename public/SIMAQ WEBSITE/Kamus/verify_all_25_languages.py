...
# -*- coding: utf-8 -*-
"""
Master Verification Script for all 25 Languages:
ID, EN, MS, FR, DE, UR, HI, BN, RU, ZH, ES, TR, PT, HA, SW, FA, JA, KO, NL, IT, BS, SQ, TH, BER, AM
"""
...
LANGS = ['id', 'en', 'ms', 'fr', 'de', 'ur', 'hi', 'bn', 'ru', 'zh', 'es', 'tr', 'pt', 'ha', 'sw', 'fa', 'ja', 'ko', 'nl', 'it', 'bs', 'sq', 'th', 'ber', 'am']

print("=" * 75)
print("MASTER VERIFIKASI 25 BAHASA APLIKASI WEB DHAMIR & HARF")
print("=" * 75)

# 1. Check all translation caches
print("\n[1/6] Memeriksa Cache Terjemahan 25 Bahasa...")
for lang in LANGS:
    if lang == 'id':
    ...
    with open(cache_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    if lang in ['bs', 'sq', 'th', 'ber', 'am']:
        assert len(data) == 6236, f"Cache {lang} hanya memiliki {len(data)} ayat (seharusnya 6,236)!"
        print(f"  [OK] {lang.upper():<4} : 6,236 / 6,236 ayat lengkap ({os.path.basename(cache_file)})")
    else:
        assert len(data) >= 450, f"Cache {lang} hanya memiliki {len(data)} ayat!"
        print(f"  [OK] {lang.upper():<4} : {len(data):,} ayat ({os.path.basename(cache_file)})")

# 2. Check dhamir_data.json
print("\n[2/6] Memeriksa Kelengkapan 299 Entri Jamid Mabny di 25 Bahasa...")
...
for lang in LANGS:
    ...
    print(f"  [OK] {upper:<4} : 299 / 299 entri Jamid Mabny 100% lengkap")

# 3. Check harf_data.json
print("\n[3/6] Memeriksa Kelengkapan 187 Entri Harf Ghair 'Amil di 25 Bahasa...")
...
for lang in LANGS:
    ...
    assert missing_surat == 0, f"Missing SuratArti{upper} in {missing_surat} entries"
    print(f"  [OK] {upper:<4} : 187 / 187 entri Harf Ghair 'Amil 100% lengkap")
...
print("\n" + "=" * 75)
print("SELURUH 25 BAHASA (TERMASUK AMAZIGH & AMHARIC) 100% TERVERIFIKASI & SIAP!")
print("=" * 75)

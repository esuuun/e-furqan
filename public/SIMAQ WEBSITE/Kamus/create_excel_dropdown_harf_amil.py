...
for r in rows[1:]:
    ...
    item_key = f"{std_b}__{nk_str}"
    if item_key not in unique_harfs:
        unique_harfs[item_key] = {
            'Bentuk': std_b,
            'No': nk_str,
            'Kata': k_str,
            'Arti': ak_str,
...
ws_dict = wb.create_sheet(title="Daftar Harf 'Amil & Surah")
...
# ----------------------------------------------------
# 2. Populate "Daftar Harf 'Amil & Surah" (For Validation & Lookup)
# ----------------------------------------------------
ws_dict.append(['Bentuk Harf List', '', 'No Kata', 'Kata Arab', 'Arti Kata', 'Frekuensi', 'Kategori', 'Lookup Key'])
...
sorted_items = sorted(list(unique_harfs.values()), key=lambda x: (int(x['Bentuk'].split('.')[0]), int(x['No']) if x['No'].isdigit() else 99))
for r_idx, info in enumerate(sorted_items, start=2):
    ws_dict.cell(row=r_idx, column=3, value=int(info['No']) if info['No'].isdigit() else info['No'])
    ...
    ws_dict.cell(row=r_idx, column=8, value=f"{info.get('Bentuk')}__{info.get('No')}")
...
ws_dashboard['B2'] = "DASHBOARD EKSPLORASI HARF 'AMIL AL-QUR'AN"
...
dv_bentuk = DataValidation(type="list", formula1=f"='Daftar Harf ''Amil & Surah'!$A$2:$A${cat_max_row}", allow_blank=False)
ws_dashboard.add_data_validation(dv_bentuk)
dv_bentuk.add(ws_dashboard['C5'])

item_max_row = len(sorted_items) + 1
dv_nokat = DataValidation(type="list", formula1=f"='Daftar Harf ''Amil & Surah'!$C$2:$C${item_max_row}", allow_blank=False)
...
ws_dashboard['B8'] = f"=IFERROR(INDEX('Daftar Harf ''Amil & Surah'!D2:D{item_max_row}, MATCH(C5&\"__\"&F5, 'Daftar Harf ''Amil & Surah'!H2:H{item_max_row}, 0)), \"-\")"
...
ws_dashboard['D8'] = f"=IFERROR(INDEX('Daftar Harf ''Amil & Surah'!E2:E{item_max_row}, MATCH(C5&\"__\"&F5, 'Daftar Harf ''Amil & Surah'!H2:H{item_max_row}, 0)), \"-\")"
...
ws_dashboard['G8'] = f"=IFERROR(INDEX('Daftar Harf ''Amil & Surah'!F2:F{item_max_row}, MATCH(C5&\"__\"&F5, 'Daftar Harf ''Amil & Surah'!H2:H{item_max_row}, 0)), \"-\")"
...
ws_dashboard['B13'] = f"=IFERROR(FILTER('Data Master'!A2:H{total_master_rows}, ('Data Master'!B2:B{total_master_rows}=C5)*('Data Master'!C2:C{total_master_rows}=F5)), \"Pilih Nomor Kata di atas untuk memuat daftar ayat\")"
...
# Save output
output_path = os.path.join(BASE_DIR, 'Harf_Amil_Interaktif.xlsx')
wb.save(output_path)
print(f"Berhasil membuat file Excel interaktif: {output_path}")
print(f"Total baris Data Master: {len(data)}, Total Kategori: {len(sorted_cats)}, Total Kata: {len(sorted_items)}")
